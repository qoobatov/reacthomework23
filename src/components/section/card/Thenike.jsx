import React from "react";
import "./Thenike.css";
import Unsplash from "./unsplash.png";

function Thenike() {
  return (
    <>
      <div className="black-page-basket">
        <div className="text-card-container-basket">
          <div className="jerseys-bestsellers-basket">The application</div>
          <div className="the-nike">The nike</div>
          <div className="big-collection-basket">app</div>
          <div className="regular-seasons-basket">
            Here you can find many interesting and useful from nike.
            <br />
            Download the application from nike.
          </div>
          <img className="unsplash" src={Unsplash} alt="" />

          <div className="training-club">
            <p className="nike-club">Nike Traning Club</p>
            <p className="workouts">Home workouts & fitness plans</p>
          </div>
          <div className="btn-view">
            <span className="view-all">View all</span>
            <svg
              width="61"
              height="9"
              viewBox="0 0 61 9"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M60.3536 4.85355C60.5488 4.65829 60.5488 4.34171 60.3536 4.14645L57.1716 0.964466C56.9763 0.769204 56.6597 0.769204 56.4645 0.964466C56.2692 1.15973 56.2692 1.47631 56.4645 1.67157L59.2929 4.5L56.4645 7.32843C56.2692 7.52369 56.2692 7.84027 56.4645 8.03553C56.6597 8.2308 56.9763 8.2308 57.1716 8.03553L60.3536 4.85355ZM0 5H60V4H0V5Z"
                fill="#FB0223"
              />
            </svg>
          </div>
        </div>
      </div>
    </>
  );
}

export default Thenike;
